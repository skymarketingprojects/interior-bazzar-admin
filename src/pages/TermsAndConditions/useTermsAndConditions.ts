...
const useTermsAndConditions = () => {
  ...
  const [termsCondition, setTermsCondition] = useState(dummyContent);
  const fetchTermsCondition = async () => {
    try {
      setLoading(true);
      const res = await PageService.getTermsConditon();
      if (!res.response) {
        return;
      }
      setTermsCondition(res.data.page_content);
    } catch (e) {
      logger.error("Error fetching termsCondition: ", e);
    } finally {
      setLoading(false);
    }
  };
