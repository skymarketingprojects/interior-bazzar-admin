...
const useDisclaimer = () => {
  ...
  const [disclaimer, setDisclaimer] = useState(dummyContent);
  const fetchDisclaimer = async () => {
    try {
      setLoading(true);
      const res = await PageService.getDisclaimer();
      if (!res.response) {
        return;
      }
      setDisclaimer(res.data.page_content);
    } catch (e) {
