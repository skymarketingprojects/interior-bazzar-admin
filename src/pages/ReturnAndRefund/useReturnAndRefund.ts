...
const useReturnAndRefund = () => {
  const [loading, setLoading] = useState(true);
  const [returnRefund, setReturnRefund] = useState(dummyContent);
  const fetchDisclaimer = async () => {
    try {
      setLoading(true);
      const res = await PageService.getReturnRefund();
      if (!res.response) {
        return;
      }
      setReturnRefund(res.data.page_content);
    } catch (e) {
      logger.error("Error fetching returnRefund", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return { loading, returnRefund };
};

export default useReturnAndRefund;
